/**
 * My code is on the verge of death //i can tell
 *
 * Website made by
...
function stars(p) {
    ...
    function draw() {
        ...
        smoothScrollY = p.lerp(smoothScrollY, window.scrollY, 0.15);
        for (var i = 0; i < starNumber; i++) { //draws the stars adjusted for paralax for each frame
            p.circle(starX[i], starY[i] + ((smoothScrollY * starSize[i]) * -0.36), starSize[i])
        }
    }
...
//bro really forgot about window.innerWidth
